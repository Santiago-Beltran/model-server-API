...
from modelserverAPI.models.usage import RawInput, ProcessedData, VALID_CITIES, VALID_PROPERTY_TYPES
...
def get_mean_price_per_sqm_in_zone(latitude: float, longitude: float) -> PositiveFloat:
    closest = sorted(data, key=lambda property: haversine((latitude, longitude), (property["latitude"], property["longitude"])))
...
def get_property_type_booleans(property_type: str) -> tuple:
    ...
    return {valid_property_type: (1 if property_type == valid_property_type else 0) for valid_property_type in VALID_PROPERTY_TYPES}

def get_city_booleans(city: str) -> dict:
    if city not in VALID_CITIES:
        return None
    return {valid_city: (1 if valid_city == city else 0) for valid_city in VALID_CITIES}
...
def transform_data(raw_input: RawInput) -> ProcessedData:
    ...
    return ProcessedData(
        zone_mean_price_per_sqm=get_mean_price_per_sqm_in_zone(raw_input.latitude, raw_input.longitude),
